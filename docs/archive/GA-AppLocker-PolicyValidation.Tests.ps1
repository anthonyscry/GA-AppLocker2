...
.NOTES
    Run: Invoke-Pester -Path .\GA-AppLocker-PolicyValidation.Tests.ps1 -Output Detailed
#>

BeforeAll {
    # Import the module
    $modulePath = Join-Path $PSScriptRoot 'GA-AppLocker-PolicyValidation.psm1'
...
Describe 'Test-AppLockerXmlSchema' {
    
    Context 'Valid Policy' {
        BeforeAll {
            $script:ValidPath = Save-TestPolicy -Content $ValidPolicy -Name 'valid'
        }
    ...
    Context 'Invalid Enforcement Mode' {
        BeforeAll {
            $script:InvalidModePath = Save-TestPolicy -Content $InvalidEnforcementModePolicy -Name 'invalidmode'
        }
        
        It 'Should fail for invalid EnforcementMode' {
            $result = Test-AppLockerXmlSchema -XmlPath $script:InvalidModePath
            $result.Success | Should -BeFalse
            $result.Errors | Should -Contain { $_ -like '*EnforcementMode*' }
        }
    }
    
    Context 'Invalid Collection Type' {
        BeforeAll {
            $script:InvalidTypePath = Save-TestPolicy -Content $InvalidCollectionTypePolicy -Name 'invalidtype'
        }
        
        It 'Should fail for invalid RuleCollection Type' {
            $result = Test-AppLockerXmlSchema -XmlPath $script:InvalidTypePath
            $result.Success | Should -BeFalse
            $result.Errors | Should -Contain { $_ -like '*InvalidType*' }
        }
    }
}
...
Describe 'Test-AppLockerRuleGuids' {
    
    Context 'Valid GUIDs' {
        BeforeAll {
            $script:ValidPath = Save-TestPolicy -Content $ValidPolicy -Name 'validguids'
        }
    ...
    Context 'Lowercase GUIDs' {
        BeforeAll {
            $script:LowercasePath = Save-TestPolicy -Content $LowercaseGuidPolicy -Name 'lowercaseguid'
        }
        
        It 'Should fail for lowercase GUIDs' {
            $result = Test-AppLockerRuleGuids -XmlPath $script:LowercasePath
            $result.Success | Should -BeFalse
            $result.Errors | Should -Contain { $_ -like '*UPPERCASE*' }
        }
    }
    
    Context 'Duplicate GUIDs' {
        BeforeAll {
            $script:DuplicatePath = Save-TestPolicy -Content $DuplicateGuidPolicy -Name 'duplicateguid'
        }
...
Describe 'Test-AppLockerRuleSids' {
    
    Context 'Valid SIDs' {
        BeforeAll {
            $script:ValidPath = Save-TestPolicy -Content $ValidPolicy -Name 'validsids'
        }
    ...
    Context 'Invalid SID Format' {
        BeforeAll {
            $script:InvalidSidPath = Save-TestPolicy -Content $InvalidSidPolicy -Name 'invalidsid'
        }
        
        It 'Should fail for malformed SID' {
            $result = Test-AppLockerRuleSids -XmlPath $script:InvalidSidPath
            $result.Success | Should -BeFalse
            $result.Errors | Should -Contain { $_ -like '*invalid SID*' }
        }
    }
    
    Context 'Missing SID' {
        BeforeAll {
            $script:MissingSidPath = Save-TestPolicy -Content $MissingSidPolicy -Name 'missingsid'
        }
        
        It 'Should fail when UserOrGroupSid is missing' {
            $result = Test-AppLockerRuleSids -XmlPath $script:MissingSidPath
            $result.Success | Should -BeFalse
            $result.Errors | Should -Contain { $_ -like '*missing*' }
        }
    }
}
...
Describe 'Test-AppLockerRuleConditions' {
    
    Context 'Valid Conditions' {
        BeforeAll {
            $script:ValidPath = Save-TestPolicy -Content $ValidPolicy -Name 'validconditions'
        }
    ...
    Context 'Invalid Hash' {
        BeforeAll {
            $script:InvalidHashPath = Save-TestPolicy -Content $InvalidHashPolicy -Name 'invalidhash'
        }
        
        It 'Should fail for invalid SHA256 hash' {
            $result = Test-AppLockerRuleConditions -XmlPath $script:InvalidHashPath
            $result.Success | Should -BeFalse
            $result.Errors | Should -Contain { $_ -like '*SHA256*' -or $_ -like '*hash*' }
        }
    }
    
    Context 'Wrong Hash Type' {
        BeforeAll {
            $script:WrongTypePath = Save-TestPolicy -Content $WrongHashTypePolicy -Name 'wronghashtype'
        }
        
        It 'Should fail for non-SHA256 hash type' {
            $result = Test-AppLockerRuleConditions -XmlPath $script:WrongTypePath
            $result.Success | Should -BeFalse
            $result.Errors | Should -Contain { $_ -like "*Type='SHA256'*" }
        }
    }
    
    Context 'Empty Publisher Name' {
        BeforeAll {
            $script:EmptyPublisherPath = Save-TestPolicy -Content $EmptyPublisherPolicy -Name 'emptypublisher'
        }
        
        It 'Should fail for empty PublisherName' {
            $result = Test-AppLockerRuleConditions -XmlPath $script:EmptyPublisherPath
            $result.Success | Should -BeFalse
            $result.Errors | Should -Contain { $_ -like '*empty PublisherName*' }
        }
    }
    
    Context 'User-Writable Paths' {
        BeforeAll {
            $script:UserWritablePath = Save-TestPolicy -Content $UserWritablePathPolicy -Name 'userwritable'
        }
        ...
        It 'Should warn about user-writable paths' {
            ...
            $result.Warnings | Should -Contain { $_ -like '*user-writable*' -or $_ -like '*Downloads*' }
        }
    }
}
...
Describe 'Invoke-AppLockerPolicyValidation' {
    
    Context 'Valid Policy - Full Pipeline' {
        BeforeAll {
            $script:ValidPath = Save-TestPolicy -Content $ValidPolicy -Name 'fullpipeline'
        }
    ...
    Context 'Invalid Policy - Full Pipeline' {
        BeforeAll {
            $script:InvalidPath = Save-TestPolicy -Content $InvalidSidPolicy -Name 'invalidpipeline'
        }
    ...
    Context 'Report Export' {
        BeforeAll {
            $script:ValidPath = Save-TestPolicy -Content $ValidPolicy -Name 'reportexport'
...
'@
        ...
        It 'Should handle policy with no rule collections' {
            ...
            $result.Success | Should -BeTrue
            $result.Warnings | Should -Contain { $_ -like '*No RuleCollections*' }
        }
    }
...
'@
        ...
        It 'Should handle multiple collections of same type' {
            ...
            # This is actually valid XML but unusual - validation should pass
            $result.Success | Should -BeTrue
        }
    }
...
<AppLockerPolicy Version="1">
  <RuleCollection Type="Exe" EnforcementMode="AuditOnly">
    <FilePublisherRule Id="A1B2C3D4-E5F6-7890-ABCD-EF1234567890" Name="日本語ルール" Description="日本語の説明" UserOrGroupSid="S-1-1-0" Action="Allow">
      <Conditions>
        <FilePublisherCondition PublisherName="O=マイクロソフト" ProductName="*" BinaryName="*">
          <BinaryVersionRange LowSection="*" HighSection="*" />
        </FilePublisherCondition>
      </Conditions>
    </FilePublisherRule>
